...
export interface NewMainnetTransactionActionConfig {
	address?: string;
}

export interface NewMainnetTransactionActionTemplate {
	from: string;
	timestamp: string;
	value: any;
	token: string;
...
class NewMainnetTransaction extends AbstractAction {
	constructor() {
		super();
		this._service = EtherscanService;
		this._name = "EtherscanNewMainnetTransaction";
		this._litteralName = "New Mainnet transaction";
		this._description = "Fetch new Mainnet transaction for ERC20 token";
	...
	// Get action config for this action
		const actionConfig: NewMainnetTransactionActionConfig = action.config;

		// Get last trigger history for this action
		const lastTriggerHistory: TriggerHistory = await this.getLastTriggerHistory(action);

		const transaction = await EtherscanService.getMainnetTransactions(actionConfig.address);
	...
	// Get last trigger history for this action (to get the fetched content)
		...
		const content: NewMainnetTransactionActionTemplate = {
...
export default new NewMainnetTransaction();